...
if(isset($_POST['gallery_sub'])){
	...
	if(isset($_POST['gallery_name'])){
		...
		if(!empty($gallery_name)){
			...
			$date=date('Y-m-d');
			mysql_query("INSERT INTO eventsalbum (AlbumName,DateAdded,SortOrder) values ('$gallery_name','$date','$gallery_sort')") or die ("insert eventsalbum error : ". mysql_error());
			$gallery_id=mysql_insert_id();
			
			$path_full="../images/events/photos/";
         ...
         $second = $_POST['second'];
            ...
            $filesecond=clean(trim($second[$ctr]));
				...
				if(!empty($datafile))
				{
					mysql_query("INSERT INTO eventsphotos (AlbumID,Caption,Credits,SortOrder,Filename,ImageThumb,ImageSecond) VALUES ('$gallery_id','$filecaption','$filecredits','$filesort','$datafile','$filethumbs','$filesecond')") or die ("photo error :".mysql_error());
					/*$photoID=mysql_insert_id();
					
					$file=$photoID."_pevent.jpg";
					$file_full=$path_full."/".$file;
					$db_file_name="images/events/photos/".$file;
					mysql_query("UPDATE eventsphotos SET Filename='$db_file_name' WHERE PhotoID='$photoID';") or die("update gall photo : ".mysql_error());
...
if(isset($_POST['image_add_sub']))
{
	...
	$add_datafile=clean(trim($_POST['image_add']));
				...
				if(!empty($add_datafile))
				{
					...
					$add_path_full="../images/events/photos/";
               $add_thumbs=clean(trim($_POST['thumb_add']));
               $add_second=clean(trim($_POST['second_add']));
					//echo $add_cap."<br />". $add_cred;
					mysql_query("INSERT INTO eventsphotos (AlbumID,Caption,Credits,SortOrder,Filename,ImageThumb,ImageSecond) VALUES ('$add_gallery_id','$add_cap','$add_cred','$add_sort','$add_datafile','$add_thumbs','$add_second')") or die ("photo error :".mysql_error());
					/*$add_photoID=mysql_insert_id();
					
					$add_file=$add_photoID."_pevent.jpg";
					$add_file_full=$add_path_full."/".$add_file;
					$db_add_file_name="images/events/photos/".$add_file;
					mysql_query("UPDATE eventsphotos SET Filename='$db_add_file_name' WHERE PhotoID='$add_photoID';") or die("update gall photo : ".mysql_error());
...
<script type="text/javascript" src="script/events_script.js"></script>
...
function imageAdd(){
	...
	$("#image_upload").append('<div id="upload_'+imagenum+'" style="display: block; margin-top:2px; padding-top: 3px; border-top: 1px solid #000000;"><table><tr><td>Caption:</td><td><input type="text" name="caption[]"><input type="button" value="Remove" onclick="imageRemove('+imagenum+')"></td></tr><tr><td>Main Image:</td><td><input type="text" name="image[]"> (600x600)</td></tr><tr><td>Thumbnail:</td><td><input type="text" name="thumbs[]"> (135x90)</td></tr><tr><td>Thumbnail:</td><td><input type="text" name="second[]"> (301x200)</td></tr><tr><td>Credits:</td><td><input type="text" name="credits[]"></td></tr><tr><td>Sort Order:</td><td><input type="text" name="sort[]" style="width: 50px"></td></tr></table></div>');
	imagenum++;
}
...
<body>
   ...
   <div id="error"></div>
   <h3> EVENTS ALBUMS</h3>
      <form action="events_photos.php" id="gallery_form" name="gallery_form" method="post" enctype="multipart/form-data">
         <table>
            <tr>
               <td>
                  Gallery Name:
				...
				<div id="image_upload">
                  ...
                  <table>
                     ...
                     <tr>
                        ...
                        <td>
                           <input type="text" name="thumbs[]"> (135x90)
                        </td>
                     </tr>
                     <tr>
                        <td>
...
$query="SELECT * FROM eventsalbum";
$results = mysql_query($query) or die (mysql_error());
$total = mysql_num_rows($results) / $page_max;


$results = mysql_query("select * from eventsalbum order by AlbumID limit $page, $page_max");
$count = 0;
$row_ctr=1;
		...
		<div style="width: 950px; background: #ccc;display: inline-block; padding-left: 4px;">
            ...
            <table>
               ...
               <tr>
                  ...
                  <td>
                     <input type="text" id="gsort_<?php echo $albumID;?>" value="<?php echo $row['SortOrder'];?>" style="width: 50px;">
                  </td>
               </tr>
            </table>
			...
			<div id="gallery_status_<?php echo $row_ctr;?>" style="float: right;">
				Status: <?php 
						...
						?>
				<input type="button" value="<?php echo $change_stat;?>" onclick="changeEventStat(<?php echo $albumID;?>)">
			</div>
		</div>
			...
			<div style="width: 978px; margin-left: 10px;">
               ...
               <table>
                  ...
                  <tr>
                     ...
                     <td>
                        <input type="text" name="image_add" id="image_add_<?php echo $albumID; ?>"> (600x600)
                        <!-- input type="file" name="image_add" id="image_add_<?php echo $albumID; ?>" -->
                  ...
                  <tr>
                     ...
                     <td>
                        <input type="text" name="thumb_add" id="thumb_add_<?php echo $albumID;?>"> (135x90)
                     </td>
                  </tr>
                  <tr>
                     <td>
                     ...
                     <td>
                        <input type="text" name="second_add" id="second_add_<?php echo $albumID;?>"> (301x200)
                     </td>
                  </tr>
                  <tr>
                     <td>
                  ...
                  <tr>
                     ...
                     <td>
                        <input type="text" name="sort_add" id="sort_add_<?php echo $albumID;?>" style="width: 50px; ">
                     </td>
                  </tr>
                  <tr>
                     <td colspan="2">
...
for ($i = 0; $i < $total; $i += 1) {
   if ($i * $page_max == $page)
      ...
      echo '<a href="events_photos.php?page=' . $i . '">' . ($i + 1) . '</a>';
