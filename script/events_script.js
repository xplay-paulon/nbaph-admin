function showInfo(id)
{
	...
	for(var a=1;a<=max;a++)
	{
		if(a==id){
			...
			$.ajax({
				url: "events_photos_content.php?id="+gallId,
				cache: false,
...
function saveInfo(sID){
	...
	gsort=$.trim(gsort);
	$.ajax({
		url: "events_update_info.php?action=info&sID="+sID+"&gname="+gname+"&gsort="+gsort,
		cache: false,
...
function updateCap(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "events_update.php?cid="+cid+"&caption="+caption,
				cache: false,
...
function updateCredits(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "events_update_credits.php?cid="+cid+"&cred="+cred,
				cache: false,
...
function updateSort(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "events_update_sort.php?cid="+cid+"&cred="+cred,
				cache: false,
				success: function(upCred){		
...
function updateThumb(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "events_update_thumbs.php?cid="+cid+"&cred="+cred+"&sec="+sec,
				cache: false,
				success: function(upCred){
...
function deleteGalleryPhoto(gid){
		...
		//alert(gid);
			$.ajax({
				url: "events_delete.php?gid="+gid,
				cache: false,
				success: function(gallDel){		
				$('#error').html(gallDel);
					window.location.href='events_photos.php';
				}
			});
...
function changeEventStat(statID){
	var stat_ctr=$("#ctr_id_"+statID).val();
	$.ajax({
		url: "event_update_status.php?statID="+statID+"&action=photos&rand="+Math.random(),
		cache: false,
