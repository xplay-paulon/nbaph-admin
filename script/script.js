function showInfo(id)
{
	...
	for(var a=1;a<=max;a++)
	{
		if(a==id){
			...
			$.ajax({
				url: "gallery_content.php?id="+gallId,
				cache: false,
...
function deleteGalleryPhoto(gid){
		...
		//alert(gid);
			$.ajax({
				url: "gallery_delete.php?gid="+gid,
				cache: false,
...
function updateCap(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "gallery_update.php?cid="+cid+"&caption="+caption,
				cache: false,
...
function updateCredits(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "gallery_update_credits.php?cid="+cid+"&cred="+cred,
				cache: false,
...
function updateSort(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "gallery_update_sort.php?cid="+cid+"&cred="+cred,
				cache: false,
				success: function(upCred){		
...
function updateThumb(cid){
		...
		//alert(gid);
			...
			$.ajax({
				url: "gallery_update_thumbs.php?cid="+cid+"&cred="+cred+"&sec="+sec,
				cache: false,
				success: function(upCred){		
...
function addToGallery(addID){

		var addcap=$('#caption_add_'+addID).val();
		var addcred=$('#credits_add_'+addID).val();
		
		alert(addID+" -- "+addcap+" -- "+addcred);
		//alert(caption);
		//alert(gid);

}
function changeStat(statID){
	var stat_ctr=$("#ctr_id_"+statID).val();
	$.ajax({
		url: "gallery_update_status.php?statID="+statID,
		cache: false,
...
function saveInfo(sID){
	...
	gdesc=$.trim(gdesc);
	$.ajax({
		url: "gallery_update_info.php?sID="+sID+"&gname="+gname+"&gdesc="+gdesc+"&gsort="+gsort,
		cache: false,
